...
export default function Profiles() {
  ...
  return (
    <div className="min-h-screen bg-[oklch(0.13_0.028_261.692)] flex flex-col items-center justify-start text-white relative pt-20 md:pt-24 pb-16 px-4">
      ...
      {/* Mobile Grid Layout */}
      <div className="grid grid-cols-2 md:hidden gap-6 w-full max-w-md">
        {profiles.map((profile) => (
          ...
          >
            ...
            />
            <span className="mt-2 text-base">{profile.name}</span>
          </div>
        ))}
        <div className="flex flex-col items-center cursor-pointer"
             onClick={() => navigate("/new-profile")}>
          <div className="w-20 h-20 rounded-full bg-gray-600 flex items-center justify-center cursor-pointer hover:opacity-80 transition-opacity">
            <span className="text-2xl">+</span>
          </div>
          <span className="mt-2 text-base">Yeni ekle</span>
        </div>
      </div>
